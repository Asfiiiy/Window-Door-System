    ...
    <div class="container">
        ...
        <form method="get" action="">
            ...
            <div class="search-filter">
                <input type="text" name="search" placeholder="Search by name or quantity..." value="<?= !empty($search) ? htmlspecialchars($search) : '' ?>">
                <select name="company">
                ...
                <a href="index.php?page=reports_hardware" style="padding: 8px 15px; background-color: #f44336; color: white; border-radius: 4px; text-decoration: none;">Reset</a>
            </div>
        </form>
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            <!-- <a href="index.php?page=edit_hardware&id=<?= $item['id'] ?>" class="btn btn-edit">Edit</a> -->
                            <a href="index.php?page=reports_hardware&delete=<?= $item['id'] ?>" class="btn btn-delete" onclick="return confirm('Are you sure you want to delete this hardware item?')">Delete</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
        ...
        <div class="pagination">
            <?php if ($page_num > 1): ?>
                <a href="index.php?page=reports_hardware&p=1&search=<?= urlencode($search) ?>&company=<?= $company_filter ?>">First</a>
                <a href="index.php?page=reports_hardware&p=<?= $page_num - 1 ?>&search=<?= urlencode($search) ?>&company=<?= $company_filter ?>">Prev</a>
            <?php endif; ?>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                ...
                <?php else: ?>
                    <a href="index.php?page=reports_hardware&p=<?= $i ?>&search=<?= urlencode($search) ?>&company=<?= $company_filter ?>"><?= $i ?></a>
                <?php endif; ?>
            <?php endfor; ?>
            
            <?php if ($page_num < $total_pages): ?>
                <a href="index.php?page=reports_hardware&p=<?= $page_num + 1 ?>&search=<?= urlencode($search) ?>&company=<?= $company_filter ?>">Next</a>
                <a href="index.php?page=reports_hardware&p=<?= $total_pages ?>&search=<?= urlencode($search) ?>&company=<?= $company_filter ?>">Last</a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
