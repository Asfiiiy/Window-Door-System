...
<body>
    <div class="container">
        ...
        <form method="get" action="index.php" id="filterForm">
            ...
            <div class="search-filter">
                <div class="filter-group">
                    <input type="text" name="search" placeholder="Client name" value="<?= htmlspecialchars($search) ?>" oninput="this.form.submit()">
                </div>
                
                <div class="filter-group">
                    <input type="text" name="qp_search" placeholder="QP number" value="<?= htmlspecialchars($qp_search) ?>" oninput="this.form.submit()">
                </div>
            ...
            <tbody>
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            <a href="index.php?page=reports_invoices&delete=<?= $quote['id'] ?>" class="btn btn-delete" onclick="return confirm('Are you sure you want to delete this quotation?')">Delete</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
        ...
        <div class="pagination">
            <?php if ($page_num > 1): ?>
                <a href="index.php?page=reports_invoices&p=1&search=<?= urlencode($search) ?>&qp_search=<?= urlencode($qp_search) ?>&company=<?= $company_filter ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>">First</a>
                <a href="index.php?page=reports_invoices&p=<?= $page_num - 1 ?>&search=<?= urlencode($search) ?>&qp_search=<?= urlencode($qp_search) ?>&company=<?= $company_filter ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>">Prev</a>
            <?php endif; ?>
            ...
            for ($i = $start; $i <= $end; $i++): ?>
                ...
                <?php else: ?>
                    <a href="index.php?page=reports_invoices&p=<?= $i ?>&search=<?= urlencode($search) ?>&qp_search=<?= urlencode($qp_search) ?>&company=<?= $company_filter ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>"><?= $i ?></a>
                <?php endif; ?>
            <?php endfor; ?>
            
            <?php if ($page_num < $total_pages): ?>
                <a href="index.php?page=reports_invoices&p=<?= $page_num + 1 ?>&search=<?= urlencode($search) ?>&qp_search=<?= urlencode($qp_search) ?>&company=<?= $company_filter ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>">Next</a>
                <a href="index.php?page=reports_invoices&p=<?= $total_pages ?>&search=<?= urlencode($search) ?>&qp_search=<?= urlencode($qp_search) ?>&company=<?= $company_filter ?>&date_from=<?= urlencode($date_from) ?>&date_to=<?= urlencode($date_to) ?>">Last</a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    </div>
