  ...
  <script>
    ...
    function createResultRow(label, value, unit, amount, isTotal = false, extraClass = '') {
      return `
        <div class="result-item ${extraClass}">
          ...
          <span class="result-value ${amount ? 'price-value' : ''}">
            ${amount ? formatCurrency(amount) : ''}
            ${!amount && value ? `<span class="no-length">${unit}</span>` : ''}
          </span>
          ${!amount && value ? `<span class="result-note">${unit}</span>` : ''}
        </div>
      `;
    }
    ...
    function calculate() {
      ...
      const outputHTML = `
        ...
        ${createResultRow('Frame Length', frameLength.toFixed(2), 'lengths', frameCost)}
        ${createResultRow('Sash Length', sashLength.toFixed(2), 'lengths', sashCost)}
        ${createResultRow('Net Sash Length', netSashLength.toFixed(2), 'lengths', netSashCost)}
        ${createResultRow('Beading Length', beadingLength.toFixed(2), 'lengths', beadingCost)}
        ${createResultRow('Interlock Length', interlockLength.toFixed(2), 'lengths', interlockCost)}
        ...
        ${createResultRow('Net', net.toFixed(2), 'sft', netCost)}
        ${createResultRow('Net Rubber', netRubber.toFixed(2), 'bundles', netRubberCost)}
        ${createResultRow('Burshi', burshi.toFixed(2), 'ft', burshiCost)}
