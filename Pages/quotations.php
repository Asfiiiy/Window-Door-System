    ...
    if (isset($_POST['save_quotation'])) {
        ...
        try {
            ...
            unset($_SESSION['calculation_started']);
             ...
             header("Location: index.php?page=report_quotation&quotation_id=".$quotation_id);
            exit();
...
<body>
  <div class="container">
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="clientSelect" name="client_id" required>
          ...
          <div id="clientDisplay" class="print-only">
            <div><strong>Name:</strong> <span id="clientNameDisplay" class="editable">Client Name</span></div>
            <div><strong>Address:</strong> <span id="clientAddressDisplay" class="editable">Client Address</span></div>
            <div><strong>Phone:</strong> <span id="clientPhoneDisplay" class="editable">Phone Number</span></div>
          </div>
        </div>
    ...
    <i class="fas fa-print"></i> Print
            ...
            <div class="btn-group">
              <button type="submit" name="reset_quotation" class="btn btn-sm reset-btn">
            ...
            <tr>
              ...
              <td>
                <input type="number" class="form-control form-control-sm no-print area-field" 
                       name="items[<?= $index ?>][area]" value="<?= number_format($item['area'], 2) ?>" step="0.01" <?= $index === 5 ? 'readonly' : '' ?>>
              ...
              <td>
                <input type="number" class="form-control form-control-sm no-print qty-field" 
                       name="items[<?= $index ?>][quantity]" value="<?= $item['quantity'] ?>" <?= $index !== 5 ? 'readonly' : '' ?>>
                <span class="print-only"><?= $item['quantity'] ?></span>
              </td>
              <td>
                <input type="number" class="form-control form-control-sm no-print rate-field" 
                       name="items[<?= $index ?>][rate]" value="<?= number_format($item['rate'], 2) ?>" step="0.01" <?= $index === 5 ? 'disabled' : '' ?>>
              ...
              <td>
                <input type="number" class="form-control form-control-sm no-print amount-field" 
                       name="items[<?= $index ?>][amount]" value="<?= number_format($item['amount'], 2) ?>" readonly>
...
<div class="window-specs-section mt-4">
    ...
    <div class="table-responsive">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <?php 
                        if (isset($item['width'])) {
                            echo number_format($item['width'], 2);
                        } else {
                            echo '<span class="text-danger">Width Missing</span>';
                        }
                        ?>
                    </td>
                    ...
                    <td>
                        ...
                        if (isset($item['height'])) {
                            echo number_format($item['height'], 2);
                        } else {
                            echo '<span class="text-danger">Height Missing</span>';
                        }
                        ?>
                    </td>

                    <td><?= number_format($item['area'] ?? 0, 2) ?></td>
                ...
                <tr>
                    <td colspan="8" class="text-center text-muted py-3">
                        No window specifications available
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
        ...
        <div class="terms-conditions">
          <div class="mb-3">
            <strong>Note:</strong> All Profile Warranty is subject to Company Policy
          </div>
          <div>
            <strong class="no-print">Terms & Conditions:</strong>
            <textarea class="form-control no-print" name="terms" rows="6" style="width: 100%;">Advance: 80%
After Delivery of Frames: 15%
After installation: 5%

1. Company will not responsible for any change in design after signing this Agreement.
2. Order will Deliver in 25 days after final Measurements.
3. Upto 5mm Gap will be Sealed Through Silicon by Company, more than that will be Client responsibility. 
4. Provision of Electricty and Scaffolding will be responsibility of Client.
5. The Prices put in this quotation are valid till 10 days.</textarea>
            <div class="print-only">
              ...
              1. Company will not responsible for any change in design after signing this Agreement.<br>
              2. Order will Deliver in 25 days after final Measurements.<br>
              3. Upto 5mm Gap will be Sealed Through Silicon by Company, more than that will be Client responsibility.<br>
              4. Provision of Electricty and Scaffolding will be responsibility of Client.<br>
              5. The Prices put in this quotation are valid till 10 days.
            </div>
          </div>
        ...
        <div class="window-types-section ">
          <h3>Select Window Types</h3>
          <div class="window-types-grid">
            <?php
            $windowTypes = [
        ...
        <input type="hidden" name="total_amount" id="totalAmountInput" value="<?= $final_total ?>">
      </form>
    </div>
  </div>
...
function printQuotation() {
    ...
    // Process window types section - only show selected ones
    const windowTypesContainer = printContents.querySelector('.window-types-section');
    if (windowTypesContainer) {
        // Clear existing content
        windowTypesContainer.innerHTML = '<h3>Selected Window Types</h3><div class="window-types-grid-print">';
        
        // Add only selected window types
        document.querySelectorAll('.window-btn.selected').forEach(btn => {
            const windowId = btn.dataset.id;
            const imgSrc = document.querySelector(`#img-${windowId}`).src;
            const label = btn.textContent.trim();
            
            windowTypesContainer.innerHTML += `
                <div class="window-type-print">
        ...
        windowTypesContainer.innerHTML += '</div>';
    }
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .quotation-table th { background-color: #2c3e50!important; color: white!important; padding: 8px; }
                .quotation-table td { padding: 8px; border-bottom: 1px solid #ddd; }
                .window-types-grid-print { 
                    display: grid; 
                    grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); 
                    gap: 15px; 
